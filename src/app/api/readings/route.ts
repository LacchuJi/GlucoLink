...
export async function GET() {
  try { const { user, patient } = await requirePatient(); const readings = await prisma.reading.findMany({ where: { patientId: patient.id }, orderBy: { recordedAt: "desc" }, take: 180 }); await audit(user.id, "READ", "Reading"); return Response.json({ readings }); } catch (error) { return errorResponse(error); }
}
export async function POST(request: Request) {
  try {
    ...
    await audit(user.id, "CREATE", "Reading", reading.id); return Response.json({ reading }, { status: 201 });
